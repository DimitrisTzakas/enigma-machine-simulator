...
logoLabel=Label(frameLogo,width=WIDTH, height=80,image=img,bg='grey20',bd=0)
...
def rotorButtonAction(rotor,arrow): #this action is also called from a keyboard press -- function keyPressAction())
...
def keyPressAction(event,x):
    PlaySound('key-Down.wav',0)
    print qwerty[x]
    s=StringVar(lightBoard, qwerty[x])
    lightBoard.itemconfigure(lightKey[x], text=s.get(),fill='yellow')
    
def placeKeyboard():
    global qwerty,keyText
    qwerty='QWERTZUIOASDFGHJKPYXCVBNML'
    key=[None]*26
    keyText=[None]*26
    keyBoard=Canvas(frameKeyboard,width=WIDTH,height=180,bg='grey16',highlightthickness=0)
    ...
    for k in range(9):
        x=cw/10*(k+1)
        coords=(x-R,y-R,x+R,y+R)
        key[k]=keyBoard.create_oval(coords,fill='black',outline='grey90',width=2,tags=qwerty[k])
        keyText[k]=keyBoard.create_text(x,y,text=qwerty[k],fill='grey90',font=('BOLD',15),tags=qwerty[k])
 #       print keyBoard.itemcget(keyText[k], 'tags')  #keyBoard.itemconfigure(I, text=S).
    x=cw/9
    y=ch/2
    R=cw/30    
    for k in range(8):
        x=cw/10*(k+1)+1.5*R
        coords=(x-R,y-R,x+R,y+R)
        key[k+9]=keyBoard.create_oval(coords,fill='black',outline='grey90',width=2,tags=qwerty[k+9])
    ...
    for k in range(9):
        x=cw/10*(k+1)
        coords=(x-R,y-R,x+R,y+R)
        key[k+17]=keyBoard.create_oval(coords,fill='black',outline='grey90',width=2,tags=qwerty[k+17])

...
#place variant
variantLabel=Label(frameSettings,text='Kriegsmarine M3',fg='silver',bg='grey20',font=("bold",13))
...
def keyReleaseAction(event,x):
    ...
    lightBoard.itemconfigure(lightKey[keyLit],fill='grey25')             #change lightText color effect return to normal from yellow to grey
...
def placePlugboard():
    ...
    for k in range(9):
        x=cw/10*(k+1)
        coords=(x-R,y-R,x+R,y+R)
        plugKey[k]=plugBoard.create_oval(coords,fill='',outline='grey90',width=2,tags=qwerty[k])

...
root.configure(width=WIDTH,height=HEIGHT,bg='black')
root.geometry('%dx%d+%d+%d' % (WIDTH, HEIGHT, x, y))
#end setting main window
frameLogo=Frame(root,width=WIDTH, height=100,bd=0)
frameLogo.grid(row=0)
frameSettings = Frame(root,width=WIDTH, height=170,bg="grey15")
frameSettings.grid(row=1,sticky='wesn')
frameLightboard = Frame(root,width=500, height=160,bg='grey15')
frameLightboard.grid(row=2,sticky='wesn')
frameKeyboard = Frame(root,width=WIDTH, height=160,bg="blue")
frameKeyboard.grid(row=3,sticky='wesn')
framePlugboard = Frame(root,width=WIDTH, height=100,bg="yellow")
framePlugboard.grid(row=4,sticky='wesn')
frameOutText = Frame(root,width=WIDTH, height=30,bg="white")
frameOutText.grid(row=5,sticky='wesn')
#place logo
img = PhotoImage(file = "enigmaLogo.gif")
logoLabel=Label(frameLogo,width=500, height=100,image=img,bg='grey20',bd=0)
...
def placeLightboard():
    lightBoard=Canvas(frameLightboard,width=100,height=100,bg='orange',highlightthickness=0)

...
def clearTextAction():
    textInVar.set('')
    textOutVar.set('')
textClearButton=Button(frameOutText,text='Clear Text',bg='grey20',fg='gold',wraplength=30,command=clearTextAction)
...
machineResetButton=Button(frameOutText,text='Reset',bg='grey20',fg='gold',wraplength=30,command=machineResetAction)
machineResetButton.place(relx=0,rely=0.5,anchor=W,height=50,width=50)

def userKeyboardPressedAction(event):
    char=(event.char).upper()
    if char in qwerty:
        newChar=keyPressAction(event,qwerty.index(char),'aKey')
        keyReleaseAction(event,qwerty.index(char))
        textInVar.set(textInVar.get()+char)
        textIn.xview_scroll(2,UNITS)
        textOutVar.set(textOutVar.get()+newChar)
        textOut.xview_scroll(2,UNITS)
...
def keyPressAction(event,x,whoCalledMe):
    ...
    if whoCalledMe=='amachineKey':     #write to text box only if this function was called by a user key
        textInVar.set(textInVar.get()+qwerty[x])  

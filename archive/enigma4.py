...
def initialize():
    ...
    for i in range(rotorsSetting[2]-1): rotateRotor(rotor1,1) #rotor up
    for i in range(rotorsRing[2]-1): rotateRotor(rotor1,-1)   #rotor down
    rotor3Display=rotorsSetting[2]
    rotor2=rotors[rotorsInstall[1]-1]
    for i in range(rotorsSetting[1]-1): rotateRotor(rotor2,1) #rotor up
    for i in range(rotorsRing[1]-1): rotateRotor(rotor2,-1) #rotor down
    rotor2Display=rotorsSetting[1]
    rotor3=rotors[rotorsInstall[0]-1]
    for i in range(rotorsSetting[0]-1): rotateRotor(rotor3,1) #rotor up
    for i in range(rotorsRing[0]-1): rotateRotor(rotor3,-1) #rotor down
...
def setRotorsInstall(rotor,selection): # from left to right
    ...
    rotorsInstall[rotor]=selection
    rotor1=rotors[rotorsInstall[2]]
    rotor2=rotors[rotorsInstall[1]]
    rotor3=rotors[rotorsInstall[0]]
    
def setRotorsSetting(rotor,s): # from left to right
    global rotor1Display, rotor2Display, rotor3Display
    if rotor==0:
        rotateRotor(rotor3,s)
        rotor1Display=rotor1Display % 26 + s
    elif rotor==1:
        rotateRotor(rotor2,s)
        rotor2Display=rotor2Display % 26 + s
    elif rotor==2:
        rotateRotor(rotor1,s)
        rotor3Display=rotor3Display % 26 + s
...
def encryptChar(x):
    ...
    return charFromPlugBoard(encryptedChar)

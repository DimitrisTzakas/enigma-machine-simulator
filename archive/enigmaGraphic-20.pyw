...
def plugPressAction(event,k):
    ...
    if (k not in plug1) and (k not in plug2): # if key is not selected
        ...
        if  (k == plug1[len(plug1)-1]) and (len(plug1)>len(plug2)):  # if key selected is not a paired one then delete it - the last selected
            plugBoard.itemconfigure(plugKey[plug1.pop()],fill='grey10')
            plugColors.pop()
            ...
            plugBoard.itemconfigure(plugKey[plug1.pop(position)],fill='grey10') # return to original state of the item poped from plug1 list
            plugBoard.itemconfigure(plugKey[plug2.pop(position)],fill='grey10') # return to original state of the item poped from plug2 list
...
def placePlugboard():
    ...
    for k in range(9):
        x=cw/10*(k+1)
        coords=(x-R,y-R,x+R,y+R)
        plugKey[k]=plugBoard.create_oval(coords,fill='black',outline='grey90',width=2,tags=qwerty[k])
    ...
    for k in range(8):
        x=cw/10*(k+1)+4*R
        coords=(x-R,y-R,x+R,y+R)
        plugKey[k+9]=plugBoard.create_oval(coords,fill='black',outline='grey90',width=2,tags=qwerty[k+9])
    ...
    for k in range(9):
        x=cw/10*(k+1)
        coords=(x-R,y-R,x+R,y+R)
        plugKey[k+17]= plugBoard.create_oval(coords,fill='black',outline='grey90',width=2,tags=qwerty[k+17])

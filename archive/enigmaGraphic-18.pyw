...
root.wm_title("Enigma Kriegsmarine M3 --- 1ο ΕΠΑΛ ΜΟΛΑΩΝ")
...
#place variant
variantLabel=Label(frameSettings,text='Kriegsmarine M3',fg='silver',bg='grey22',font=("bold",11))
...
def keyPressAction(event,x,whoCalledMe):
...
def placeKeyboard():
    ...
    for k in range(26): keyBoard.tag_bind(qwerty[k],'<Button-1>',lambda event,x=k: keyPressAction(event,x,'amachineKey'))
...
def plugPressAction(event,k):
    global colorPointer,numOfPlugsSelected
    colors=['deep sky blue','spring green','orchid1','orange','red','purple','maroon','blue2','khaki3','brown4','magenta2','plum2','lightBlue2']
    if k not in plugsIndex:
        if len(plugsIndex)<26:
            plugsIndex.append(k)
            numOfPlugsSelected=len(plugsIndex)
            if numOfPlugsSelected == 1: colorPointer=0
            plugBoard.itemconfigure(plugKey[k],fill=colors[colorPointer])
            if numOfPlugsSelected % 2 == 0: colorPointer+=1
    else:
         plugBoard.itemconfigure(plugKey[k],fill='grey10')
         if numOfPlugsSelected % 2 == 0:
             colorPointer-=1
             numOfPlugsSelected-=1
         plugsIndex.pop()

    print plugsIndex
    N=numOfPlugsSelected
    if N % 2 == 1: N-=1
    for i in range(0,N/2,2):
        couple=qwerty[plugsIndex[i]]+qwerty[plugsIndex[i+1]]
        print couple
    
def placePlugboard():
    global plugBoard,plugKey,plugsIndex
    plugsIndex=[] # list of selected plug indexes to be used by plugPressAction(event,k) - default null
    plugKey=[None]*26
    plugBoard=Canvas(framePlugboard,width=WIDTH,height=100,bg='black',highlightthickness=0)
    ...
    for k in range(9):
        x=cw/10*(k+1)
        coords=(x-R,y-R,x+R,y+R)
        plugKey[k]=plugBoard.create_oval(coords,fill='grey10',outline='grey90',width=2,tags=qwerty[k])
        plugBoard.create_text(x-2.5*R,y,text=qwerty[k],fill='white',font=('BOLD',11))
    x=cw/9
    y=ch/2
    R=cw/70    
    for k in range(8):
        x=cw/10*(k+1)+4*R
        coords=(x-R,y-R,x+R,y+R)
        plugKey[k+9]=plugBoard.create_oval(coords,fill='grey10',outline='grey90',width=2,tags=qwerty[k+9])
        plugBoard.create_text(x-2.5*R,y,text=qwerty[k+9],fill='white',font=('BOLD',11))
    x=cw/9
    y=ch/1.25
    R=cw/70    
    for k in range(9):
        x=cw/10*(k+1)
        coords=(x-R,y-R,x+R,y+R)
        plugKey[k+17]= plugBoard.create_oval(coords,fill='grey10',outline='grey90',width=2,tags=qwerty[k+17])
        plugBoard.create_text(x-2.5*R,y,text=qwerty[k+17],fill='white',font=('BOLD',11))
    for k in range(26): plugBoard.tag_bind(qwerty[k],'<Button-1>',lambda event,x=k: plugPressAction(event,x))    

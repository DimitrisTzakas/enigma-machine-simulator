...
root.wm_title("Enigma Kriegsmarine M3 ---by 1ο ΕΠΑΛ ΜΟΛΑΩΝ")
...
#------------------------------------------------------------------------
def textWindowEncrypt(event,textIn,textOut):
...
def createTextWindow(event):
    ...
    textIn = Text(textFrame1, height=20, width=30,bg='BLACK',fg='yellow',font=('courier',14),wrap=WORD)
    textOut= Text(textFrame2, height=20, width=30,bg='BLACK',fg='yellow',font=('courier',14),wrap=WORD)
    ...
    buttonFrame=Frame(textWindow,height=60,width=textWindow.winfo_width(),bg='grey10')
    buttonFrame.grid(row=1,column=0,columnspan=2)

    cypherButton=Button(buttonFrame,text='Κρυπτογράφησε--->',bg='grey50',command=lambda :textWindowEncrypt(event,textIn,textOut))
...
variantLabel=Label(frameSettings,text='Kriegsmarine M3 1o ΕΠΑΛ ΜΟΛΑΩΝ',fg='lightSkyBlue1',bg='grey20',font=logoFont,wraplength=180)

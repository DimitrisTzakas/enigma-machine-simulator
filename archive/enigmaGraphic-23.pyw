...
def textWindowEncrypt(event,textIn,textOut):
    s=textIn.get(1.0,END)
    s=enigma.encryptPhrase(s[:len(s)-1]) # must omit last character because it is not valid
...
def createTextWindow(event):
    ...
    cypherButton=Button(buttonFrame,text='Κρυπτογράφησε--->',command=lambda :textWindowEncrypt(event,textIn,textOut))

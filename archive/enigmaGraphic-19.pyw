...
def removePlugBoardCables():
    ...
    enigma.plugBoard=[]
    for i in range(26): plugBoard.itemconfigure(plugKey[i],fill='grey10')
    plug1=[]
    plug2=[]
    plugColors=[]
...
def plugPressAction(event,k):
    ...
    if (k not in plug1) and (k not in plug2): # if key is not selected
        if len(plug1) == len(plug2):  # if a pair exists take a new color
            ...
            plug2.append(k)
            enigma.plugBoard.append(qwerty[plug1[len(plug1)-1]]+qwerty[plug2[len(plug2)-1]]) #update enigma machine plugboard
    else:     # if a key is allready selected
        if (k in plug1) and (len(plug2) != len(plug1)):  # if key selected is not a paired one then delete this one but first find it
            position=plug1.index(k)
            plugBoard.itemconfigure(plugKey[plug1.pop(position)],fill='grey10')
            ...
            plugBoard.itemconfigure(plugKey[plug1.pop(position)],fill='grey10') 
            plugBoard.itemconfigure(plugKey[plug2.pop(position)],fill='grey10')
            plugColors.pop(position)
...
def placePlugboard():
    ...
    for k in range(9):
        ...
        plugBoard.create_text(x-2.5*R,y,text=qwerty[k],fill='white',font=('BOLD',13))
    x=cw/9
    y=ch/2
    R=cw/70    
    for k in range(8):
        ...
        plugBoard.create_text(x-2.5*R,y,text=qwerty[k+9],fill='white',font=('BOLD',13))
    x=cw/9
    y=ch/1.25
    R=cw/70    
    for k in range(9):
        ...
        plugBoard.create_text(x-2.5*R,y,text=qwerty[k+17],fill='white',font=('BOLD',13))
